...
{\bfseries{The algorithm is described in the paper\+: "{}\+Tracking objects with generic calibrated sensors\+: an algorithm based on color and 3\+D shape features"{}, please cite it if you use the tracker in your research.}}

You can watch a video of the tracker in action \href{http://www.youtube.com/watch?v=Xp8qUhfMzhU}{\texttt{ here}}.\hypertarget{group__src__demoYoga_lib_sec}{}\doxysubsection{\texorpdfstring{Libraries}{Libraries}}\label{group__src__demoYoga_lib_sec}

\begin{DoxyItemize}
\item YARP libraries.
\item Open\+Cv
\end{DoxyItemize}\hypertarget{group__src__demoYoga_parameters_sec}{}\doxysubsection{\texorpdfstring{Parameters}{Parameters}}\label{group__src__demoYoga_parameters_sec}
...
\DoxyCodeLine{\textcolor{preprocessor}{\#}}
\DoxyCodeLine{likelihoodThreshold\ \ \ \ \ \ \ \ \ 0.005}

\end{DoxyCode}
\hypertarget{group__src__demoYoga_portsa_sec}{}\doxysubsection{\texorpdfstring{Ports Accessed}{Ports Accessed}}\label{group__src__demoYoga_portsa_sec}
The tracker need to be connected to a port that streams images, at the very least, in order to work.\hypertarget{group__src__demoYoga_portsc_sec}{}\doxysubsection{\texorpdfstring{Ports Created}{Ports Created}}\label{group__src__demoYoga_portsc_sec}
...
\end{DoxyItemize}\hypertarget{group__icub__pf3dTracker_in_files_sec}{}\doxysubsection{\texorpdfstring{Input Data Files}{Input Data Files}}\label{group__icub__pf3dTracker_in_files_sec}
The tracker requires three input files\+: an image file used to build the colour model of the tracked ball, a file defining the motion model applied to the estimate of the ball between frames, and a shape model for the ball, defining its size and the distance between inner and outer contours (see the paper cited in the description section). For more detail on these files, please have a look at the parameters section.\hypertarget{group__icub__pf3dTracker_out_data_sec}{}\doxysubsection{\texorpdfstring{Output Data Files}{Output Data Files}}\label{group__icub__pf3dTracker_out_data_sec}
The module produces one output file that is only useful for debugging (current\+\_\+histogram.\+csv) and can be configured to save the images it produces to files, but this is not recommended\+: use yarpdatadumper instead.\hypertarget{group__src__demoYoga_tested_os_sec}{}\doxysubsection{\texorpdfstring{Tested OS}{Tested OS}}\label{group__src__demoYoga_tested_os_sec}
